...
class FlightProgram:
    def __init__(self):
        ...
        self.tree = ttk.Treeview(self.frame, columns = ('Distance','Carrier','Aircraft','Origin','Destination','Month','Success Rates'))
    ...
    def generate_preset_table(self,root):
        ...
        self.resultLabel.grid(row=self.table_row-1,column=1)
    ...
    def insert_table(self,df,root):
        ...
        self.tree.heading('#2', text="Carrier Name", command=lambda:self.treeview_sort_column(self.tree, "Carrier", False))
        self.tree.heading('#3', text="Aircraft", command=lambda:self.treeview_sort_column(self.tree, "Aircraft", False))
        self.tree.heading('#4', text="Origin", command=lambda:self.treeview_sort_column(self.tree, "Origin", False))
        self.tree.heading('#5', text="Destination", command=lambda:self.treeview_sort_column(self.tree, "Destination", False))
        self.tree.heading('#6', text="Month", command=lambda:self.treeview_sort_column(self.tree, "Month", False))
        self.tree.heading('#7', text="Rank", command=lambda:self.treeview_sort_column(self.tree, "Rank", False))
        ...
        self.tree.column('#5', width = 175)
        self.tree.column('#6', width = 60)
        self.tree.column('#7', width = 175)
        ...
        for index,row in df.iterrows():
            self.tree.insert('', 'end', text=index,values = (row['DISTANCE'], row['UNIQUE_CARRIER_NAME'],row['AIRCRAFT'],row['ORIGIN_CITY_NAME'], row['DEST_CITY_NAME'], row['MONTH'], row['RANKS']) )
        ...
        self.countLabel.grid(row = self.table_row+5,column = 1)
...
#DISTANCES 
        distanceLabel = tk.Label(self.root,text = "Travel Distance").grid(column=0, row=self.table_row-8)
        distances = ttk.Combobox(self.root, values=distancesList)
        distances.grid(column=0, row=self.table_row-7)
        distances.bind("<<ComboboxSelected>>", filterDistance)

#ORIGIN CITY NAMES
        originCityNamesLabel = tk.Label(self.root, text = "Choose Origin City Name").grid(column=0, row=self.table_row-6)
        originCityNames = ttk.Combobox(self.root, values=originCityNamesList)
        originCityNames.grid(column=0, row=self.table_row-5)
        originCityNames.bind("<<ComboboxSelected>>", filterOriginCityName)

#ORIGIN STATE NAMES
        originStateNamesLabel = tk.Label(self.root, text = "Choose Origin State Name").grid(column=0, row=self.table_row-4)
        originStateNames = ttk.Combobox(self.root, values=originStateNamesList)
        originStateNames.grid(column=0, row=self.table_row-3)
        originStateNames.bind("<<ComboboxSelected>>", filterOriginStateName)

#DEST CITY NAMES
        destinationCityNamesLabel = tk.Label(self.root, text = "Choose Destination City Name").grid(column=1, row=self.table_row-8)
        destinationCityNames = ttk.Combobox(self.root, values=destCityNamesList)
        destinationCityNames.grid(column=1, row=self.table_row-7)
        destinationCityNames.bind("<<ComboboxSelected>>", filterDestCityName)

#DEST STATE NAMES
        destinationStateNamesLabel = tk.Label(self.root, text = "Choose Destination State Name").grid(column=1, row=self.table_row-6)
        destinationStateNames = ttk.Combobox(self.root, values=destStateNamesList)
        destinationStateNames.grid(column=1, row=self.table_row-5)
        destinationStateNames.bind("<<ComboboxSelected>>", filterDestStateName)


#MONTHS
        monthsLabel = tk.Label(self.root,text = "Choose Month").grid(column=1, row=self.table_row-4)
        months = ttk.Combobox(self.root, values=monthsList)
        months.grid(column=1, row=self.table_row-3)
        months.bind("<<ComboboxSelected>>", filterMonth)

#YEARS
        yearsLabel = tk.Label(self.root,text = "Choose Year").grid(column=2, row=self.table_row-8)
        years = ttk.Combobox(self.root, values=yearsList)
        years.grid(column=2, row=self.table_row-7)
        years.bind("<<ComboboxSelected>>", filterYear)

#CARRIER NAME
        carrierNamesLabel = tk.Label(self.root,text = "Choose Carrier Name").grid(column=2, row=self.table_row-6)
        carrierNames = ttk.Combobox(self.root, values=carrierNamesList)
        carrierNames.grid(column=2, row=self.table_row-5)
        carrierNames.bind("<<ComboboxSelected>>", filterCarrierName)

#AIRCRAFT NAME
        aircraftNamesLabel = tk.Label(self.root,text = "Choose Aircraft Name").grid(column=2, row=self.table_row-4)
        aircraftNames = ttk.Combobox(self.root, values=aircraftNamesList)
        aircraftNames.grid(column=2, row=self.table_row-3)
